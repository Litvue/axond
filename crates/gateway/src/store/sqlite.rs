...
impl Store for SqliteStore {
    ...
    async fn mark_provider_models_stale_unless_source(
        ...
        self.with_conn(move |conn| {
            // `fetched_at` is fixed-width RFC3339 UTC; SQLite's default
            // BINARY collation makes `<` chronological.
            conn.execute(
...
mod tests {
    ...
    /// A replica still on the old URL runs the pre-fetch stale mark against a
    /// row another replica just wrote for the new URL. That row is still being
    /// refreshed, so it is neither staled nor freed for the old listing.
    #[tokio::test]
    async fn provider_models_lagged_source_does_not_stale_or_replace_a_live_row() {
        let store = SqliteStore::open(":memory:").expect("memory sqlite");
