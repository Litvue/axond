...
| `catalog.aliases.json` | One provider publishing one model under two callable ids, as `qiniu-ai` does upstream. Both stay offerings: the authored key joins the neutral record, the other keeps the id a request to that provider uses. |
