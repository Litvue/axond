...
| `drift.*.json` | One rejected payload each: malformed JSON, the wrong document shape, a missing required field, a type change, an unrecognized enumerated value, an id that does not match its key, and every way a price can be unusable. None of them may replace last-known-good state. |
