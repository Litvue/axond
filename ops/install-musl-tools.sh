...
run_bounded_apt() {
    # Run timeout as root with apt so it can terminate the complete command on
    # expiry. `sudo -n` makes an unexpected credential prompt fail immediately.
...
self_test() {
    local actual expected attempts sleeps status problems=0
    ...
    if ((status != 0)) || ((attempts != 2)) || [[ $sleeps != 5 ]]; then
        echo "self-test: transient failure used status=$status attempts=$attempts sleeps='$sleeps'" >&2
        problems=1
    fi
    ...
    status=0
    install_musl_tools >/dev/null 2>&1 || status=$?
    if ((status == 0)) || ((attempts != APT_ATTEMPTS)) || [[ $sleeps != "5 10" ]]; then
        echo "self-test: persistent failure used status=$status attempts=$attempts sleeps='$sleeps'" >&2
        problems=1
    fi
