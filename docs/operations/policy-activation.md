...
[ADR 0050](../adr/0050-runtime-policy-activation.md).
...
| Every cap the document states is at least `1` | A cap of zero denies every request for the scope, which is not a bound on spending but a closed scope wearing one — the bootstrap file refuses it for the same reason, so a published document may not express what an authored one may not. Close a scope through tenancy (withdraw the projection, revoke its credentials), not through a limit | an authored document is refused with `validation`, naming `subject_limit_microdollars` or `namespace_limit_microdollars`; one an earlier build already stored still hydrates, and its activation is refused with `invalid_policy`, naming `budget_limit_microdollars` or `namespace_budget_limit_microdollars`, while the replica keeps the last policy it activated |
