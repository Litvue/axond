...
| `cleanup` | Upstream responses opened and still open when the caller was gone, how long the release took, and whether the process exited cleanly on `SIGTERM`. A stalled buffered attempt is counted as well as a stream, so a header- or body-bound row proves the abandoned response was released rather than reporting a zero nothing ever contributed to. The release is timed from the moment the caller was gone — before the row waits for anything else, including its usage records — and gated on a ceiling, so a body held for seconds is recorded as having taken them rather than as having been released the instant the harness looked, and a body freed only by shutdown fails the row. |
