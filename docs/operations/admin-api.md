  ...
  "resource": {
    ...
    "content_middleware": [{
      ...
      "guardrail": {
        ...
        "rules": [
          {"id": "deny-private-key", "pattern": "-----BEGIN PRIVATE KEY-----", "action": "block"},
...
The identifier must name middleware compiled into the running binary.
`axond.redact` is the shipped deterministic content guardrail. It is always
`fail_closed`: both the administrative parser and snapshot compiler reject a
fail-open registration. `key_env` names an environment variable containing a
canonical padded-base64 32-byte key. The key is resolved only while compiling a
snapshot, then namespace-separated before generating request-local opaque
tokens; providers never receive the original matched text. Treat each `key_env`
name as a versioned, immutable reference: rotate by publishing a new policy that
names a new environment variable, not by changing the value behind an existing
name in place. The encrypted compiled cache retains only a non-secret fingerprint
of the resolved namespace key and refuses cold recovery if that reference now
resolves differently. `block` refuses before provider dispatch, while `redact`
restores only tokens generated by that same request. Missing or malformed key
material, invalid rules, an unknown
identifier, or an unsupported scope rejects the candidate and leaves the active
last-known-good snapshot serving.

Rules inspect string content and JSON member names recursively. Root `model` and
`stream` remain gateway-owned and are neither matched nor mutated. Root
`previous_response_id` and forwarded native wire headers are matched but never
rewritten: any block or redaction match refuses the request atomically so policy
cannot change continuation or protocol semantics. Matches split between those
protected values or between a protected value and the body are refused too. A
matching JSON member name
or a match spanning the route's canonical provider-visible text sequence across
fields, messages, or text parts (including non-text media separators) refuses
for the same reason. Malformed `stream` and `previous_response_id` values are
invalid requests. Identically named nested values are ordinary content and
remain mutable unless the member name itself matches a rule.

Streaming restoration requires `stream_event` and a `buffered_response_routes`
opt-in for Native Messages and Responses. The gateway validates strict transport
EOF and runs a payload-free finalizer before releasing buffered bytes or a normal
terminal marker. An incomplete request-generated token therefore becomes a typed
middleware stream error without releasing buffered provider content. Core stages
remain unregistrable: `core_stages` and identifiers such as `authentication` are
validation errors.

A block-only guardrail declares validation rather than response mutation, so an
opted-in byte-faithful route releases the provider's original bytes after strict
validation. The presence of any `redact` rule selects reconstructed output so
restored text can replace placeholders. Declassification is allowlisted to known
display-text fields for the authenticated Chat, Messages, or Responses route.
Structured tool arguments, URL fields, identifiers, names, metadata, and protocol
controls remain tokenized. A URL-looking or executable instruction embedded
inside an allowed display string is still display text and is restored; clients
must treat it as untrusted model output and must not auto-fetch, execute, or
privileged-render it. Use block-only policy when that trust boundary is not
acceptable.
