...
| `otlp_trace_context_exported` | Every replica-dedicated receiver observed at least one trace batch, proving the qualification instrumentation was exercised by the full fleet. |
| `otlp_trace_export_identity_mismatches` | The settled exact caller traces decoded by the replica-dedicated receivers equal the complete caller trace ledger. Usage-bearing identities and exact retried drain-attempt evidence are disclosed separately, so neither a missing trace, substituted exemption, delayed arrival, nor unexplained extra trace can hide. |
