...
| `503 middleware_unavailable` | Fail-closed middleware failed, exceeded its end-to-end deadline, timed out waiting for bounded blocking capacity, or runs on a replica whose blocking-middleware runtime was quarantined after an abandoned invocation. | Middleware warning logs, `axond.middleware.capacity_wait`, and `axond.middleware.capacity_timeouts`. If the warning names quarantine, repair the implementation and restart the replica; it will not schedule more middleware work. |
