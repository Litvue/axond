...
`an_owned_alias_is_listed_and_routable_only_by_its_namespace` — a name
one tenant publishes may not be enumerated or invoked by another. Configured
`[[model]]` aliases (and the file-level owner checks that went with them) were
withdrawn in ADR 0063; routing is `provider-id/model-id`. The
