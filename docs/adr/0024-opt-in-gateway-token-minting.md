...
exceed the configured authority. The `scope` ceiling is mandatory, and a
requested capability must also be held by the minting key itself; the ceiling
narrows the key's authority and cannot widen it. An omitted field inherits its
configured ceiling; omission must never widen a token into an unrestricted one.
The namespace is derived from the authorized static caller and cannot be
