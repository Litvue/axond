...
| `container_url` | absolute URL | — | **Object storage only; required.** Credential-free container URL with exactly one unescaped container path segment. Production requires HTTPS. User info, query strings (including SAS), and fragments are rejected. |
