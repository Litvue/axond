...
| `400` | `middleware_response_incompatible` | A streaming `/v1/messages` or `/v1/responses` request selected an applicable response mutator without explicitly opting that route into buffering. Refused before admission or provider dispatch. | Add the route to the governing policy's `buffered_response_routes` only if callers accept reconstructed, delayed output; otherwise remove the stream mutator. |
