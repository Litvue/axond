...
| `axond.settlement.failures` | counter | `axond.settlement.reason` | Settlements that did not record their spend: `queue_timeout` (outlived `settlement_queue_wait_ms`), `execution_timeout` (outlived `settlement_timeout_ms`), `panicked`, `cancelled` (the task was aborted or the runtime stopped under it), `refused` (background work that carried no admission reservation met a saturated replica). None is retried, because a budget charge is not idempotent. The spend-loss counter of settlement. |
...
| `axond.store.acquire_wait` | histogram (ms) | `axond.store.backend`, `axond.store.operation` | Time a Store operation waited for a connection before any SQL ran: the blocking-executor dispatch and the single connection mutex on `sqlite`, the pool permit on `postgres`. This is the contention phase; sustained growth at a flat `axond.store.query_duration` means the Store is queued on, not slow. Recorded for every outcome, including a saturated pool. |
...
| `axond.store.operations` | counter | `axond.store.backend`, `axond.store.operation`, `axond.store.outcome` | Store operations by outcome: `ok`, `error`, or `saturated` (Postgres pool permit not granted within its wait bound). The count behind the two histograms. |
| `axond.store.connections_opened` | counter | `axond.store.backend` | Successful Postgres sessions, including reconnects after the pool shed idle sessions; a failed `connect()` does not increment. SQLite opens once and does not count. A count that rises with each traffic burst is a pool that drains between them. |
