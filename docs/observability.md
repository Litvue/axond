...
| `503` | `middleware_unavailable` | A fail-closed content middleware invocation failed, exceeded its end-to-end deadline, could not acquire bounded blocking capacity before that deadline, or the process's blocking-middleware runtime was quarantined after an abandoned invocation. | Check middleware failure logs plus `axond.middleware.capacity_wait` and `axond.middleware.capacity_timeouts`. A quarantined replica schedules no more middleware work; repair the implementation and restart it before retrying. |
