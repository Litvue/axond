...
| `axond.store.acquire_wait` | histogram (ms) | `axond.store.backend`, `axond.store.operation` | Time a Store operation waited for a connection before any SQL ran: the dispatch slot (then the connection mutex) on `sqlite`, the pool permit on `postgres` plus a fresh connect when nothing idle was available. SQLite waiters queue on that slot rather than occupying one blocking thread each. Recorded for every outcome, including a saturated dispatch. |
