...
dispatch; it is not a namespace hold. An unlimited token omits the claim: a
ceiling of `0` admits nothing, because any priced request estimates above it.
Estimates are whole microdollars, so a cheap request against a cheap alias can
estimate `0` and pass any ceiling. It is enforced at admission time and emitted
by `axond mint --max-request-microdollars`.
